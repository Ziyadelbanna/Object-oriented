...
public class DropDB implements SingleQuery{
    ...
    private void validateDropDB() throws ArrayIndexOutOfBoundsException , StringIndexOutOfBoundsException{
	if(arr[0].toLowerCase().equals("drop"))
	    if(arr[1].toLowerCase().equals("database"))
	         stage4(2);
	    else {}
	//	error.reportError();
	else {}
	...
	if(pos != arr.length-1)
	{
	  //  error.reportError("drop DATABASE works only with one database");
	    return;
	}
	...
	SingleQuery x = new DropDB();
	x.setQuery("Drop daTabase mmm;");
