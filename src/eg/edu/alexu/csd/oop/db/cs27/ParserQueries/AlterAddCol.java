...
public class AlterAddCol implements SingleQuery{
    ...
    private void validateAlterAddCol() throws ArrayIndexOutOfBoundsException , StringIndexOutOfBoundsException{
	...
	if(!arr[pos].toLowerCase().equals("add") || !arr[pos+1].toLowerCase().equals("column")){
	    //error.reportError();
	    return;
	}
	
	if(pos+4 != arr.length){
	   // error.reportError("you should write only the column name and it's"
		   // + " datatype");
	    return;
	}
	parameters.add(arr[pos+2]);
	parameters.add(arr[pos+3]);
	
	//check the datatype
	if(!types.isSupported(arr[pos+3])){
	   // error.reportError("Error in your datatype \""+arr[pos+3]+"\"");
	    return;
	}
    }
    
    public static void main(String args[]){
	SingleQuery x = new AlterAddCol();
	x.setQuery("alter table xxx add yyyy date;");
